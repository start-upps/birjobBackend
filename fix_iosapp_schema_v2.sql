...
SELECT 
    u.id as user_id,
    COALESCE(ks.keywords::jsonb, u.match_keywords, '[]'::jsonb),
    COALESCE(ks.location_filters, '{}'::jsonb),
    COALESCE(ks.source_filters::jsonb, '[]'::jsonb),
    COALESCE(ks.is_active, TRUE),
    COALESCE(ks.created_at, CURRENT_TIMESTAMP),
    COALESCE(ks.updated_at, CURRENT_TIMESTAMP)
FROM iosapp.users_new u
LEFT JOIN iosapp.keyword_subscriptions ks ON ks.device_id IN (
